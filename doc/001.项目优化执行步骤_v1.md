...
- [ ] 修改`@Controller`装饰器，移除`:platform`参数
  ```typescript
...
- [ ] 修改`remove`方法，添加`@HttpCode(204)`装饰器
  ```typescript
  @Delete(":id")
...
- [ ] 修改`logout`方法，添加`@HttpCode(204)`装饰器
  ```typescript
  @Post("logout")
...
- [ ] 修改所有接口，移除`:platform`参数
- [ ] 使用`@Headers("x-platform")`获取platform参数
- [ ] 统一接口命名（使用连字符）
- [ ] 使用标准HTTP方法
- [ ] 添加`@HttpCode(204)`到删除接口
...
- [ ] 修改`ApiResult`类，添加`timestamp`字段
  ```typescript
