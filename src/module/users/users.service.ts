...
export class UsersService extends BaseService {
  ...
  async create(
    ...
    try {
      ...
      createUserDto.password = await bcryptService.encryptStr(
      ...
      const data = await this.userRepository.find({
        where: {
          ...
          account: findUserDto?.account
            ? ILike(`%${findUserDto.account}%`)
            : undefined,
          nickName: findUserDto?.nickName
            ? ILike(`%${findUserDto.nickName}%`)
            : undefined,
          email: findUserDto?.email
            ? ILike(`%${findUserDto.email}%`)
            : undefined,
          phone: findUserDto?.phone
            ? ILike(`%${findUserDto.phone}%`)
            : undefined,
        },
        order: {
          ...order,
  ...
  async update(
    ...
    try {
      ...
      // 修改密码加密
      if (updateUserDto.password) {
        userInfo.password = await bcryptService.encryptStr(
  ...
  async logIn(
    ...
    try {
      ...
      const status = await bcryptService.validateStr(
        logInDto.password,
        data.password,
      );
      if (!status) {
      ...
      const { password, deletedAt, platform: _userPlatform, ...info } = data;
